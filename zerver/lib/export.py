...
def export_with_admin_auth(realm, response, include_invite_only=True, include_private=True):
    ...
    user_subscription_query = Subscription.objects.filter(user_profile__realm=realm,
    ...
    stream_subscription_query = Subscription.objects.filter(user_profile__realm=realm,
    ...
    if include_private:
        ...
        realm_huddle_subs = Subscription.objects.select_related("recipient").filter(recipient__type=3, user_profile__realm=realm)
...
def export_messages(realm, user_profile_ids, recipient_ids,
    ...
    # in your realm to a recipient object you're exporting (that is
    # thus also in your realm).
    #
    # TODO: In theory, you should be able to export messages sent to
    # streams in your realm by cross-realm bots, as well as export
    # messages in cross-realm PM threads.
    message_query = Message.objects.filter(sender__realm=realm,
                                           sender__in=user_profile_ids,
    ...
    while True:
        ...
        output = {'zerver_message': message_chunk,
                  'zerver_usermessage': user_message_chunk}
...
def fix_foreign_keys(data, table, field_name):
    for item in data[table]:
...
def import_uploads(bucket_name, import_dir, avatar_bucket=False):
    conn = S3Connection(settings.S3_KEY, settings.S3_SECRET_KEY)
    bucket = conn.get_bucket(bucket_name, validate=True)

    records_filename = os.path.join(import_dir, "records.json")
    with open(records_filename) as records_file:
        records = ujson.loads(records_file)
...
def do_import_realm(import_dir):
    ...
    realm.save()
    bulk_import_model(data, Client, 'zerver_client')
    ...
    fix_foreign_keys(data, 'zerver_userprofile', 'bot_owner')
    ...
    for user_profile_dict in data['zerver_userprofile']:
        ...
        # TODO: Fix permissions.  Ideally just eliminate Guardian to remove this.
    ...
    fix_foreign_keys(data, 'zerver_subscription', 'user_profile')
    ...
    fix_foreign_keys(data, 'zerver_userpresence', 'user_profile')
    fix_foreign_keys(data, 'zerver_userpresence', 'client')
    bulk_import_model(data, UserPresence, 'zerver_userpresence')

    ## Import uploaded files and avatars
    # import_uploads(settings.S3_AVATAR_BUCKET, os.path.join(import_dir, "avatars"), avatar_bucket=True)
    #import_uploads(settings.S3_AUTH_UPLOADS_BUCKET, os.path.join(import_dir, "uploads"))
    ...
    while True:
        ...
        fix_foreign_keys(data, 'zerver_message', 'sender')
        fix_foreign_keys(data, 'zerver_message', 'recipient')
        fix_foreign_keys(data, 'zerver_message', 'sending_client')
        ...
        fix_foreign_keys(data, 'zerver_usermessage', 'user_profile')
